                ...
                ->arrayNode('browsers')
                    ...
                    ->arrayPrototype()
                        ->children()
                            ...
                            ->scalarNode('min_node_version')->defaultValue('20.0.0')->info('Minimum Node.js version required to launch the server')->end()
                            ->scalarNode('channel')->defaultNull()->info('Browser distribution channel, e.g. chrome-beta or msedge')->end()
                            ->scalarNode('screenshot_dir')->defaultNull()->info('Default directory where page screenshots are saved')->end()
                            ->scalarNode('downloads_dir')->defaultNull()->info('Directory where downloads are stored')->end()
                            ->scalarNode('videos_dir')->defaultNull()->info('Directory where context videos are recorded')->end()
                            ->arrayNode('tracing')
                                ->info('Record a Playwright trace for contexts created from this configuration')
                                ->children()
                            ...
                            ->end()
                            ->arrayNode('proxy')
                                ->info('Proxy applied to the browser')
                                ->children()
                                    ->scalarNode('server')->isRequired()->cannotBeEmpty()->info('Proxy server, e.g. http://host:port')->end()
                                    ->scalarNode('username')->defaultNull()->end()
                                    ->scalarNode('password')->defaultNull()->end()
                                    ->scalarNode('bypass')->defaultNull()->info('Comma-separated hosts to bypass')->end()
                                ->end()
                            ->end()
                        ->end()
                    ->end()
